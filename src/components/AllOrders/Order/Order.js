...
const Order = (props) => {
    ...
    const handleOrderStatus = (id) => {
        ...
        if(isConfirm){
            axios.put(`https://damp-cliffs-56350.herokuapp.com/services`,{id})
                    ...
                    <div className="row border border-warning w-100">
                        <div className="col-lg-6 p-5 ourservice-article">
                            ...
                            <p  className="m-0 mt-4">
                                <button onClick={() => handleDelete(_id)} className="btn btn-warning btn-lg rounded-0 me-3">Delete</button>
                                {(orderStatus === "Approved") ? "": <button onClick={() => handleOrderStatus(_id)} className="btn btn-warning btn-lg rounded-0">Approve</button> }
                            </p>
                        </div>
